...
describe("Paywall", () => {
  ...
  it("lists what the purchase unlocks — the toggle defaults to the lifetime tab", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
  ...
  it("offers a toggle between the free experience and what the lifetime unlock adds", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);

    // Both segments are real, visible controls, not a hidden picker.
    expect(getByText(t("adsModeLabel"))).toBeTruthy();
    expect(getByText(t("lifetimeModeLabel"))).toBeTruthy();

    // Defaults to the lifetime tab, so the benefits are visible without a tap.
    expect(getByText(t("feat1Title"))).toBeTruthy();
    expect(getByText(t("feat2Title"))).toBeTruthy();
  });

  it("switches the panel content when the toggle changes — it is one panel, not two cards", async () => {
    const { getByText, queryByText } = await renderWithProviders(<Paywall />);

    await fireEvent.press(getByText(t("adsModeLabel")));
    expect(getByText(t("adsModeDesc"))).toBeTruthy();
    expect(queryByText(t("feat1Title"))).toBeNull();
    expect(queryByText(t("feat2Title"))).toBeNull();

    await fireEvent.press(getByText(t("lifetimeModeLabel")));
    expect(getByText(t("feat1Title"))).toBeTruthy();
    expect(queryByText(t("adsModeDesc"))).toBeNull();
  });

  it("communicates the selected segment to a screen reader via accessibilityState", async () => {
    const { getByLabelText } = await renderWithProviders(<Paywall />);

    expect(
      getByLabelText(t("lifetimeModeLabel")).props.accessibilityState?.selected,
    ).toBe(true);
    expect(
      getByLabelText(t("adsModeLabel")).props.accessibilityState?.selected,
    ).toBe(false);

    await fireEvent.press(getByLabelText(t("adsModeLabel")));
  ...
  it("tells the user this account has nothing to restore, in poursort's own wording", async () => {
    const restore = jest.fn().mockResolvedValue("none");
    seed({ restore });
    const { getByText } = await renderWithProviders(<Paywall />);
    await fireEvent.press(getByText(t("restorePurchases")));
